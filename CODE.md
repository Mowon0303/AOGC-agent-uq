...
aogc_uq/
  ...
  fusion/               # late-fusion + 选择性控制(已自测)
    late_fusion.py      #   rank_fuse/mean_fuse(免训练)+ LogRegFusion(拟 2-4 标量)
    control.py          #   decide_action + success_vs_budget 曲线/面积(H4)
experiments/
  blindspot_demo.py     # 端到端 H1 演示(合成数据)
  franq_vs_aogc.py      # §3.5b FRANQ 漏检对比(合成数据)
  mirage_smoke.py       # 真 MIRAGE 观察 -> AOGC(loader 闭环冒烟)
  fusion_demo.py        # H2 融合 + H4 选择性控制(合成数据)
notebooks/
  colab_rollout.ipynb   # ★ Colab: clone MIRAGE -> 4-bit rollout -> judge -> 打分 jsonl 存 Drive
tests/                  # metrics/aogc/franq/mirage/rollout/fusion(共 38);全 CPU 可跑
...
| late-fusion(逻辑回归拟标量)+ 选择性控制(H4) | ✅ 实现 + 自测(`fusion/`)。rank/mean 免训练融合 + `LogRegFusion`(拟 n_signals 个系数,带正交贡献 coef_);`success_vs_budget` 成功率-问人预算曲线 + `decide_action`。合成 demo:融合后检测 AUROC↑、fused triage 达 oracle |
